...
begin
	...
	while (not eof(maestro) ) do 
		begin
			read(maestro,rMae); 
			writeln ('Codigo usuario: ',rMae.codigoUsuario, ' fecha: ',rMae.fecha, ' TiempoTotal: ',rMae.tiempoTotalSesiones:2:2 ); 
		end; 
	close(maestro); 
end; 	
...
begin
	...
	for i:= 1 to cantDetalles do 
		begin
			if (vRegDet[i].codigoUsuario < min.codigoUsuario) then 
				begin
					if (vRegDet[i].fecha <min.fecha) then 
						begin
							min := vRegDet[i]; 
						    posMin:= i;
						end;
		...
		while (min.codigoUsuario <> valorAlto) do 
			begin
				...
				while (usuarioActual = min.codigoUsuario) do   
					...
					writeln('Usuario actual: ',usuarioActual);
